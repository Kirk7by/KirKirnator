...
namespace ExpertCore
{
    ...
    internal class OperatingMechanism : IMechanism
    {
        ...
        }   //TODO: WARNING: Не работает откат назад, так как удаляет полностью
        //Удаляем наш вопрос, на который ответили "не знаю" ВНИМАНИЕ!!! ОТКАТИТЬ НА ВОПРОС НЕ ЗНАЮ НЕЛЬЗЯ!
        //ВНИМАНИЕ!!! ПОСЛЕ ЭТОГО МЕТОДА ВЫЗЫВАЕМ Безболезненный GetQuestion()
        #endregion
